...
//		t.Helper()
//		newdb, closeFunc, err := db.NewDatabase("test.db")
//		if err != nil {
...
func TestWebServer(t *testing.T) {
	...
	value1, err := db1.GetKey("adjective")
	if err != nil {
		t.Fatalf("adjective key error: %v", err)
	}

	want1 := "value-adjective"
	if !bytes.Equal(value1, []byte(want1)) {
		t.Errorf("Unexpected value of adjective key: got %q, want %q", value1, want1)
	}

	value2, err := db2.GetKey("adverb")
	if err != nil {
		t.Fatalf("adverb key error: %v", err)
	}

	want2 := "value-adverb"
	if !bytes.Equal(value2, []byte(want2)) {
		t.Errorf("Unexpected value of adverb key: got %q, want %q", value2, want2)
	}
}
